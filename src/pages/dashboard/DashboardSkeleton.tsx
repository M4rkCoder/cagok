    ...
    <div className="pt-2 px-6 max-w-7xl mx-auto h-full space-y-4">
      ...
      <Card className="overflow-hidden border-none shadow-md bg-white mb-2">
        <div className="grid grid-cols-1 lg:grid-cols-12">
          {/* Left Side (Summary) */}
          <div className="lg:col-span-4 p-4 flex flex-col justify-between h-[250px] lg:h-auto">
            <div>
              <div className="flex items-center justify-between pb-8">
                <Skeleton className="h-6 w-32 bg-slate-100" /> {/* Title */}
                <Skeleton className="h-6 w-16 rounded-full bg-slate-100" /> {/* Badge */}
              </div>
            ...
            <div className="mt-3 pb-5">
              <Skeleton className="h-10 w-full rounded-md bg-slate-100" /> {/* Footer */}
            </div>
          </div>
          ...
          <div className="lg:col-span-8 p-4 bg-slate-50/30">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 h-full">
              ...
              <div className="flex flex-col h-full">
                <Skeleton className="h-5 w-24 mb-4 bg-slate-200/50" /> {/* Title */}
                <Skeleton className="flex-1 w-full rounded-md bg-slate-200/50" /> {/* Big Block */}
              </div>
      ...
      <div className="w-full">
        {/* Mobile Tabs */}
        <div className={cn("flex w-full mb-4 gap-4", "min-2xl:hidden")}>
           {[1, 2, 3, 4].map((i) => (
