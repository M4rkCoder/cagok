...
export function SummaryCards() {
  ...
  const isEmpty =
    ...
    stats.netIncome.total === 0 &&
    stats.fixedExpense.total === 0;
    ...
    >
      ...
      <CardContent className="flex flex-row justify-between items-stretch h-60 p-0">
        ...
        <div className="flex-1 flex flex-col justify-around py-3 pl-8">
          {items.map((item: any) => (
            <div key={item.label} className="space-y-1">
              ...
              <p
                className={cn(
                  "text-3xl font-black tracking-tighter leading-none h-8",
                  item.textColor
                )}
              >
                {item.data.total !== 0 && (
        ...
        <div className="flex items-center space-x-12 px-10 py-6 m-4 mr-6 bg-black/[0.03] dark:bg-white/[0.03] rounded-xl border border-white/40 shadow-inner">
          {items.map((item: any) => {
            const hasData = item.data.max !== 0 || item.data.min !== 0 || item.data.average !== 0;
            return (
              <div
                key={item.label}
                className={cn(
                  "flex flex-col items-center h-full relative group",
                  !hasData && "opacity-0 pointer-events-none"
                )}
              >
                {/* 메인 세로축 */}
